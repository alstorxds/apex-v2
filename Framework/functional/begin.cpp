...
bool scrollbar_ex(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    const float scrollbar_center_offset = 11.0f;
    const float grab_center_offset = 6.0f;
      
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x - grab_center_offset, ImLerp(bb.Min.y + GetStyle().WindowPadding.y, bb.Max.y, grab_v_norm), bb.Max.x - grab_center_offset, ImLerp(bb.Min.y - GetStyle().WindowPadding.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
   
    draw->add_rect_filled(window->DrawList, grab_rect.Min + ImVec2(grab_center_offset - 2, 0), grab_rect.Max - ImVec2(grab_center_offset - 2, 0), gui->get_clr(clr->c_child.layout), 100.f);
