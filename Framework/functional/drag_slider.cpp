...
template<typename T>
bool slider_scalar(std::string_view label, ImGuiDataType data_type, T* p_data, const T* p_min, const T* p_max, const char* format)
{
    ...
    const float rect_size = 30;

    const ImVec2 pos = window->DC.CursorPos;

    const ImRect rect(pos + ImVec2(0, 6), pos + ImVec2(width, rect_size));
    const ImRect slider(pos + ImVec2(0, rect_size - 3), pos + ImVec2(width, rect_size - 1));
    ...
    bool hovered = ItemHoverable(slider, id, g.LastItemData.InFlags), held, pressed = ButtonBehavior(ImRect(rect.Min + ImVec2(0, 0), rect.Max - ImVec2(0, 0)), id, &hovered, &held, NULL);

    ImRect grab_bb;
    const bool value_changed = slider_behavior(ImRect(slider.Min + ImVec2(3, 0), slider.Max + ImVec2(10, 0)), id, data_type, p_data, p_min, p_max, format, NULL, & grab_bb);
    char value_buf[64];

    if (value_changed) MarkItemEdited(id);

    c_slider<T>* state = gui->anim_container(&state, id);
    state->grab_slide = ImLerp(state->grab_slide, grab_bb.Min.x - slider.Min.x, gui->fixed_speed(set->c_element.speed_animation) );
    state->alpha = ImLerp(state->alpha, hovered ? 1.f : 0.f, gui->fixed_speed(set->c_element.speed_animation) );
    ...
    state->circle = easing->im_ease(id, IsItemActive(), 3.f, 4.f, gui->fixed_speed(set->c_element.speed_animation), elastic);

    draw->add_rect_filled(window->DrawList, slider.Min, slider.Max, gui->get_clr(clr->c_element.widget_layout), 100.f);
    draw->add_rect_filled(window->DrawList, slider.Min, { slider.Min.x + state->grab_slide + 5, slider.Max.y }, gui->get_clr(clr->c_important.accent_clr), 100.f);

    draw->add_circle_filled(window->DrawList, { slider.Min.x + state->grab_slide, slider.GetCenter().y }, slider.GetHeight() + state->circle, gui->get_clr(clr->c_important.white_clr), 100.f);
    ...
    if constexpr (std::is_arithmetic_v<T>) {
        ...
        T step = (target_value - state->slow_value) * gui->fixed_speed(set->c_element.speed_animation);
    ...
    draw->render_text(set->c_font.inter_regular[0], window->DrawList, rect.Min, rect.Max, gui->get_clr(clr->c_text.text_active), value_buf, NULL, NULL, { 1.0, 0.0 }, NULL);

    if (var->c_selection.selection_alpha == 0)
    {
        state->text = clr->c_text.text;
        state->grab_slide = 0;
        state->slow_value = 0;
        state->circle = 0;
    }
...
bool c_widget::slider_float(std::string_view label, float* v, float v_min, float v_max, const char* format)
{
    return slider_scalar<float>(label, ImGuiDataType_Float, v, &v_min, &v_max, format);
}

bool c_widget::slider_int(std::string_view label, int* v, int v_min, int v_max, const char* format)
{
    return slider_scalar<int>(label, ImGuiDataType_S32, v, &v_min, &v_max, format);
}
