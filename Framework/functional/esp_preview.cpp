...
void esp_preview::initialize_preview(const ImVec2& pos, const ImVec2& size)
{
    fill_box(pos, size);
    fill_areas();
    fill_buffers();
...
void esp_preview::render_box(int type)
{
    ...
    draw->add_line(GetWindowDrawList(), box.position, box.position + ImVec2(0, box.size.y / 3), gui->get_clr(clr->c_important.white_clr));
    draw->add_line(GetWindowDrawList(), box.position, box.position + ImVec2(box.size.x / 3, 0), gui->get_clr(clr->c_important.white_clr));

    draw->add_line(GetWindowDrawList(), box.position + ImVec2(box.size.x * 2 / 3, 0), box.position + ImVec2(box.size.x, 0), gui->get_clr(clr->c_important.white_clr));
    draw->add_line(GetWindowDrawList(), box.position + ImVec2(box.size.x, 0), box.position + ImVec2(box.size.x, box.size.y / 3), gui->get_clr(clr->c_important.white_clr));

    draw->add_line(GetWindowDrawList(), box.position + ImVec2(box.size.x, box.size.y * 2 / 3), box.position + box.size, gui->get_clr(clr->c_important.white_clr));
    draw->add_line(GetWindowDrawList(), box.position + box.size, box.position + box.size - ImVec2(box.size.x / 3, 0), gui->get_clr(clr->c_important.white_clr));

    draw->add_line(GetWindowDrawList(), box.position + ImVec2(box.size.x / 3, box.size.y), ImVec2(box.position.x, box.position.y + box.size.y), gui->get_clr(clr->c_important.white_clr));
    draw->add_line(GetWindowDrawList(), ImVec2(box.position.x, box.position.y + box.size.y), box.position + ImVec2(0, box.size.y * 2 / 3), gui->get_clr(clr->c_important.white_clr));
}
